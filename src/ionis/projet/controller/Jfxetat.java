...
public class Jfxetat implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        String query;
        query = "SELECT c.idclasse, libelle, e.matricule, e.nom, e.prenom, e.email, codeclasse, telephone, " +
                "filiere, etat, COUNT(s.NBREHEURE) as nbreheure FROM participer as p, etudiant as e, " +
                "classe as c, seance as s WHERE p.MATRICULE = e.MATRICULE and e.IDCLASSE = c.IDCLASSE and " +
                "s.IDSEANCE = p.IDSEANCE and p.etat = 0 GROUP BY e.MATRICULE";
        try{
    ...
    public void chargementTableau(String query){
        ...
        try {
            ...
            while(rs.next()){
                observableList.add(new Etat(rs.getInt("idclasse"), rs.getString("libelle"),
                        ...
                        rs.getString("filiere"), rs.getInt("nbreheure")));
            }
        ...
        colEtat.setCellValueFactory(new PropertyValueFactory<>("nbreHeureAbsence"));
...
//    }
    ...
    public void onrecherche(KeyEvent keyEvent) {
        String query ;
        query = "SELECT c.idclasse, libelle, e.matricule, e.nom, e.prenom, e.email, codeclasse, telephone, filiere, " +
                "etat, COUNT(s.NBREHEURE) as nbreheure FROM participer as p, etudiant as e, classe as c, " +
                "seance as s WHERE p.MATRICULE = e.MATRICULE and e.IDCLASSE = c.IDCLASSE and s.IDSEANCE = p.IDSEANCE " +
                " and s.DATEDEBUT BETWEEN '" + idDateDebut.getValue() + "' and '" + idDateFin.getValue() + "' " +
                " and c.idclasse = " + listClasseCRUD.get(txtClasse.getSelectionModel().getSelectedIndex()) +
                " and p.etat = 0 and (e.NOM LIKE '" + idRechercher.getText() + "%' or e.PRENOM LIKE '" + idRechercher.getText() + "%' " +
                " or c.LIBELLE LIKE '" + idRechercher.getText() + "%') GROUP BY e.MATRICULE";
    ...
    public void onValider(ActionEvent actionEvent) {
        ...
        query = "SELECT c.idclasse, libelle, e.matricule, e.nom, e.prenom, e.email, codeclasse, telephone, filiere, " +
                "etat, COUNT(s.NBREHEURE) as nbreheure FROM participer as p, etudiant as e, classe as c, " +
                "seance as s WHERE p.MATRICULE = e.MATRICULE and e.IDCLASSE = c.IDCLASSE and s.IDSEANCE = p.IDSEANCE " +
                "  and p.etat = 0 and s.DATEDEBUT BETWEEN '" + idDateDebut.getValue() + "' and '" + idDateFin.getValue() + "' " +
                " and c.idclasse = " + listClasseCRUD.get(txtClasse.getSelectionModel().getSelectedIndex()) +
